<template>
    <div class="top">
        <Navbar />
        <h1 class="header left">Waiting Operator | <span class="minis" v-if="listEq.length">Total {{ listEq.length }} unit</span></h1>
        <div class="outbox">
...
<script>
    ...
    export default{
        ...
        methods: {
            loadData(data){
                let filtered = data.filter(item => item.isOperated === false)
